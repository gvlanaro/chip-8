...
public class Emulator
{
    ...
    public Emulator(string rom_path)
    {
        ...
        var rom = File.ReadAllBytes(rom_path);
    ...
    private byte Delay_Timer { get; set; }
    ...
    {
        var OpCode =
            (ushort)((Memory[PC] << 8) | Memory[PC + 1]); // combines 2 bytes (instructions for chip8 are 16bits)
        var fNibble = (byte)(OpCode >> 12); // first nibble (4-bit value)
        var x = (byte)((OpCode & 0x0F00) >> 8); // A 4-bit value, the lower 4 bits of the high byte of the instruction 
        var y = (byte)((OpCode & 0x00F0) >> 4); // A 4-bit value, the upper 4 bits of the low byte of the instruction 
        var n = (byte)(OpCode & 0x000F); // A 4-bit value, the lowest 4 bits of the instruction
        var kk = (byte)(OpCode & 0x00FF); // An 8-bit value, the lowest 8 bits of the instruction 
        var nnn = (ushort)(OpCode & 0x0FFF); // A 12-bit value, the lowest 12 bits of the instruction 
    ...
    private void IFx65(byte x)
    {
        ...
        for (var i = 0; i <= x; i++) V[i] = Memory[I + i];
        I += (byte)(x + 1);
    }
    ...
    private void IFx55(byte x)
    {
        ...
        for (var i = 0; i <= x; i++) Memory[I + i] = V[i];
        I += (byte)(x + 1);
    }
    ...
    private void IDxyn(byte X, byte Y, byte N)
    {
        ...
        for (var row = 0; row < N; row++)
        {
            ...
            var sprite_data =
                Memory[I + row]; // alternative: BitArray sprite_data = new BitArray(new byte[] { Memory[I + row] });
            var iX = 0;
            for (var pixel = 7; pixel >= 0; pixel--)
            {
                ...
                var shifted = sprite_data >> pixel; // get most significant bit

                if ((shifted & 1) != 0)
                {
                    if (Display[cX, cY] == 1) V[0xF] = 1;
